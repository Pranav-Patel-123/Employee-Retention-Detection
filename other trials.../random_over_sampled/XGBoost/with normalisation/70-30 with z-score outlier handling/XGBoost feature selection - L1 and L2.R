...
# Perform Elastic Net (L1 and L2) feature selection
x <- as.matrix(train_matrix[, -1])  # Exclude the response variable
y <- as.numeric(train_data$Attrition) - 1

elastic_net_model <- cv.glmnet(x, y, alpha = 0.5, family = "binomial")

# Extract the selected features
elastic_net_selected_features <- coef(elastic_net_model, s = elastic_net_model$lambda.min)
elastic_net_selected_features <- as.numeric(names(elastic_net_selected_features)[elastic_net_selected_features != 0])

# Include the intercept (if present)
if (length(elastic_net_selected_features) > 0) {
  elastic_net_selected_features <- elastic_net_selected_features + 1
}

# Subset the data with selected features
elastic_net_selected_data <- data[, c("Attrition", colnames(train_matrix)[elastic_net_selected_features])]
...
# Train the XGBoost model using the subset of data with Elastic Net-selected features
