...
backward_elimination <- function(data) {
  features <- names(data)[!names(data) %in% "Attrition"]
  remaining_features <- features
  while (length(remaining_features) > 1) {
    model <- glm(Attrition ~ ., data = data[, c(remaining_features, "Attrition")], family = "binomial")
    ...
    if (max(p_values) > 0.05) {
      remaining_features <- remaining_features[remaining_features != least_significant_feature]
    } else {
      break
    }
  }
...
# Perform backward elimination for feature selection
selected_features <- backward_elimination(train_data)
